...
Perkin Elmer XRD0820 Detector, with TIFF plugin
...
import numpy
import epics
from epics.devices.ad_base import AD_Base
...
class PEXRD0820(epics.Device):
    """
    support for Perkin Elmer XRD0820 AreaDetector, with TIFF plugin
    """
    cam_attrs = ('AcquireTime', 'NumImages',  
                 'ImageMode', 'TriggerMode', 'Acquire',
                 'ArrayCounter', 'ArrayCounter_RBV',
                 'BinX', 'BinY', 
                 'ArrayCallbacks',
                 'ShutterControl',
                 'PEAcquireOffset', 'PEAcquireGain')
    img_attrs = ('EnableCallbacks',)

    _nonpvs  = ('_prefix', '_pvs', '_delim',
                'cam_attrds')

    def __init__(self, prefix, filesaver='TIFF1'):
