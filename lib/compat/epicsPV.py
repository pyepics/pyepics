...
class epicsPV(PV):
   """
   This class subclasses PV to provide a compatible API to Mark Rivers epicsPV class
   ...
   def __init__(self, pvName=None, wait=True):
      ...
      Keywords:
         ...
         wait:
            ...
            wait for connection for the PV.  If wait==0 and pvName != None then
            the PV will eventually connect...
   ...
   def __getCallback(self,pvname=None,value=None,**kw):
      self.monitorState = True

   def setMonitor(self):
      """
      Sets a simple callback routine for value change eventss to note when a change occurs.
      """
      self.monitorState = False
      self.add_callback(callback=self.__getCallback)
   ...
   def putWait(self, value):
      """ put PV value, waits for the callback to  occur before it returns.  """
